             ...
             int num_repeats) 
{
    ...
    const arm_compute::TensorShape src_shape(w, h, c, n);
    src.allocator()->init(arm_compute::TensorInfo(src_shape, 1, arm_compute::DataType::F32));

    // Initialize weights, bias and output tensors
    const arm_compute::TensorShape weights_shape_conv0(static_cast<long unsigned int>(filter_w), static_cast<long unsigned int>(filter_h), static_cast<unsigned int>(src_shape.z()), k);
    const arm_compute::TensorShape biases_shape_conv0(weights_shape_conv0[3]);
    const arm_compute::TensorShape out_shape_conv0(out_w, out_h, weights_shape_conv0[3]);

    weights0.allocator()->init(arm_compute::TensorInfo(weights_shape_conv0, 1, arm_compute::DataType::F32));
    biases0.allocator()->init(arm_compute::TensorInfo(biases_shape_conv0, 1, arm_compute::DataType::F32));
    out_conv0.allocator()->init(arm_compute::TensorInfo(out_shape_conv0, 1, arm_compute::DataType::F32));

    /* [Configure functions] */
    conv0.configure(&src, &weights0, &biases0, &out_conv0, arm_compute::PadStrideInfo(wstride, hstride, pad_w, pad_h));
